...
struct WorkoutHistoryScreen: View {
    ...
    private var filteredSessions: [WorkoutSession] {
        WorkoutHistoryFiltering.filteredSessions(
            sessions: sessions,
            completedOnly: completedOnly,
            routineFilterName: routineFilterName,
            exerciseFilterId: exerciseFilterId,
            allowExerciseFilter: ( { if case .all = filter { return true } else { return false } }() ),
            searchText: searchText
        )
    }

    private var groupedDays: [(day: Date, sessions: [WorkoutSession])] {
        WorkoutHistoryFiltering.groupedDays(calendar: calendar, sessions: filteredSessions)
    }
